...
sigma_amp = [0.1:0.1:0.5 1:1:5];
num_instance = 10;

phasic_dur = 50;


prs_mtr_error = zeros(num_instance,length(sigma_amp));

for  instance_ind = 1:num_instance
    
    instance_ind
    fname = sprintf("netInstances//%s//netInstance1000Prep%d.mat",instance_folder,instance_ind);
    load(fname);
    ...
    for sigma_amp_ind = 1:length(sigma_amp)

        sigma_amp_ind 
        test_sigma_noise = sigma_amp(sigma_amp_ind);%*(tau_c+Tau_n)/Tau_n;
        ...
        for init_ind = 1:8
            ...
            sigma_noise = test_sigma_noise*(tau_c+Tau_n)/Tau_n;
            noise1 = wgn(Nch,int32(dur/dt),sigma_noise, 'linear');  
            sigma_noise2 = test_sigma_noise*(5+Tau_n)/Tau_n;
        ...
        prs_mtr_error(instance_ind,sigma_amp_ind) = ...
...
fname = sprintf("perturbResults%s",instance_folder)
save(fname,'prs_mtr_error','mean_error','sigma_amp')

disp("perturb analysis for net is done")
